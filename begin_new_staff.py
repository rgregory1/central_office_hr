...
if len(worksheet_data) == 0: 
    ...
    gc = pygsheets.authorize(outh_file='client_secret.json')
    fresh_copy_wkb = gc.open_by_key('1hdn-UkFtsUCGGIJnaUSBExOFlOs8YiPG9Ql47FG-4z4')
    fresh_copy_sheet = fresh_copy_wkb.worksheet_by_title("Original")

    # open CO New Staff Process google sheet
    staff_process_wkb = gc.open_by_key('1KWLOYV7wQjEaD0A107gZlivZ3sr8OeOcDP3OjVOSX6E')
    ...
    for staff in worksheet_data:
        ...
        individual_record_sheet = staff_process_wkb.add_worksheet(staff_name, src_worksheet=fresh_copy_sheet)
